...
- Existing saved books remain compatible. In the editor, **Fill missing details** retries description, subjects, binding, and edition for the saved ISBN. It fills only blank fields and preserves existing values and edits made during the lookup. Press **Save book** to keep the results; closing or canceling discards them. Missing data can still be entered manually. Books are not automatically looked up again. All four fields are included in CSV exports, with subjects separated by line breaks inside one quoted cell.
...
Upload the complete updated app (including `js/`, `index.html`, and `sw.js`), not just the HTML. Version 4 fetches fresh local assets when installing its offline cache. Let the hosted app load online, then close all its browser tabs and installed app windows and reopen it. Do not clear website data: that deletes the catalog. If new fields remain empty, use **Fill missing details** and save the book. For example, ISBN `9780006540465` (Troubles) returned subjects and a work description from Open Library when checked on 2026-09-13, but its matched edition supplied neither binding nor an edition label.
